...
real*8,allocatable ::pop(:,:),pop_surf(:,:),pop_amp(:,:),vib_en(:),pop1(:,:),pop_metal(:,:)
...
!!others
real*8,allocatable::q_vib(:,:),H_diab_vib(:,:),si_adiab_neu(:,:),si_adiab_neu_prev(:,:),delH_dels_vib(:,:,:)
real*8,allocatable::V_k_vib(:),v_k_vib_old(:),q2_vib(:,:),phi_gr(:,:),phi_gr_old(:,:),phi_ex(:,:),phi_ex_old(:,:)
real*8,allocatable::q_vib_gr(:,:),q_vib_ex(:,:)
...
subroutine setup
  implicit none
  character st_ch
  integer i,size_seed,seed2(2),seed_out,j
  ...
  allocate(pop(nquant,i),pop_surf(nquant,i),pop_amp(nquant,i),vib_en(i),pop1(2,i),pop_metal(nmetal,i))
  ...
  allocate(q_vib(nvib,nvib),H_diab_vib(nvib,nvib),si_adiab_neu(nvib,nvib),si_adiab_neu_prev(nvib,nvib))
  allocate(delH_dels_vib(nvib,nvib,nclass),V_k_vib(nvib),V_k_vib_old(nvib),q2_vib(nvib,nvib))
  allocate(phi_gr(ndvr,nvib),phi_ex(ndvr,nvib),phi_gr_old(ndvr,nvib),phi_ex_old(ndvr,nvib),q_vib_gr(nvib,nvib),q_vib_ex(nvib,nvib))
...
!call draw_pes
  ...
  call cpu_time(t2);tim_tot=t2-t1
  call files(1)
write(500,*)tim_tot
!write(400,*)tim_tot
...
subroutine init_cond
  ...
  real*8 energy_0
complex*16::rho_tmp(nvib,nvib),ci_diab(nvib,nvib),ci_adiab(nvib,1)
real*8::pop_diab(nvib)
...
ci(:)=si_adiab_neu(state(1),:)
!ci_diab=matmul(ci,transpose(si_adiab_neu))

!ci_adiab(:,1)=ci(:)
!rho_tmp=matmul(conjg(ci_adiab),ci_adiab)
!do i=1,nvib
!rho_tmp(i,i)=0.d0
!enddo
!rho_tmp(state(vib_el),state(vib_el))=1.d0

!ci_diab=matmul(si_adiab_neu,matmul(rho_tmp,si_adiab_neu))
!do i=1,nvib
!write(6,*)i,cdabs(ci_diab(i,i))**2
!enddo
!stop
...
subroutine evolve(nsteps)
  ...
  iterm=0
  do i=1,nsteps
!write(200+traj_num,'(1es14.5,20i)')curr_time*1.d12,state
    ...
    outerloop: do j=2,n_el
      do k=nvib+1,2*nvib
        ...
        if(V1*V2<0.d0) then
          ...
          delF=abs(delH_dels(state(1),state(1),1)-delH_dels(k,k,1))
          !call frank_condon(i,k-nvib,fc)
          fc=dot_product(phi_gr(:,state(1)),phi_ex(:,k-nvib))
          p_LZ=abs(2*pi*Vc(state(j)-2*nvib)**2/(hbar*v(1)*delF)) * fc**2
         call random_number(rnd)
!write(200,*)'1',p_LZ,rnd 
          if(rnd<p_LZ) then
            state_tentative=state
            state_tentative(j)=k
            state_tentative(1)=0
    ...
    outerloop2: do j=2*nvib+1,nquant
      if(.not.(any(j==state))) then
        do k=1,nvib
          ...
          if(V1*V2<0.d0) then
!            delF = 2*g_coup2 * g_coup1/(mass(1)*omg1_prime**2)
            delF=abs(delH_dels(state(i),state(i),1)-delH_dels(k,k,1))
            !call frank_condon(k,state(i)-nvib,fc)
            fc=dot_product(phi_gr(:,k),phi_ex(:,state(i)-nvib))
            p_LZ=abs(2*pi*Vc(j-2*nvib)**2/(hbar*v(1)*abs(delF))) * fc**2
            call random_number(rnd)
!write(200,*)'2',p_LZ,rnd 
             if(rnd<p_LZ) then
                state_tentative=state
                state_tentative(i)=j
                state_tentative(1)=k
...
subroutine average(i)
  implicit none
  integer,intent(in) :: i
  integer j,i1,j1,k,kp,k1,k2
  complex*16 ci_diab(nvib,nvib),rho_ad(nquant,nquant),ci_adiab(nvib,1)
  complex*16 rho_tmp(nvib,nvib)
...
enddo
        ...
        if(state(1)>0)then
                rho_tmp(state(vib_el),state(vib_el))=1.d0
                ci_diab=matmul(si_adiab_neu,matmul(rho_tmp,si_adiab_neu))
                pop1(1,j)=pop1(1,j)+1.d0
                do k1=1,nvib
                        pop(k1,j)=pop(k1,j)+cdabs(ci_diab(k1,k1))
                enddo
        else
                pop1(2,j)=pop1(2,j)+1.d0
                rho_tmp(state(vib_el)-nvib,state(vib_el)-nvib)=1.d0
                ci_diab=matmul(si_adiab_neu,matmul(rho_tmp,si_adiab_neu))
                do k1=1,nvib
                        pop(k1+nvib,j)=pop(k1+nvib,j)+cdabs(ci_diab(k1,k1))
                enddo
        endif
!do i=1,n_el
!        if(state
...
!        do k1=1,nvib
!                pop(k1,j)=pop(k1,j)+cdabs(ci(k1))**2
!        enddo
!      else
!        do k=1,n_el
...
subroutine save_old_state
  ...
  phi_old=phi
V_k_vib_old=V_k_vib
si_adiab_neu_prev=si_adiab_neu
...
subroutine evolve_quantum_small_dtq
  ...
  dVk_dt=(V_k_vib-V_k_vib_old)/dtc

  dtq1=0.02/maxval(vdotd)
  dtq2=0.02*hbar/maxval(V_k_vib-sum(V_k_vib)/real(nvib))
  dtq=dtq1
...
subroutine rk4(ci,dtq,dvk_dt)
  ...
  V_k_vib=V_k_vib+dVk_dt*dtq/2.d0
  call compute_mat_adiab
...
subroutine compute_mat_adiab
  ...
  mat_adiab=-vdotd
  V_avg=sum(V_k_vib)/real(nvib)
  do i=1,nvib
    mat_adiab(i,i)=mat_adiab(i,i)-iota/hbar*(V_k_vib(i)-V_avg)
  enddo
...
subroutine hop
  ...
  if(flag_hop==1) then
    state_tentative=state
    en_diff=V_k_vib(vib_st)-V_k_vib(vib_tentative)
...
write(600,*)(i-1)*nstep_avg*dtc*1.d15,pop1(:,i)
   do k=1,2*nvib
...
subroutine tise
  ...
  real*8 si_adiab_old(nquant,nbasis),hamil_vib(nvib,nvib)
  real*8 q0,ens_vib(nvib),vect_vib(nvib,nvib),q_1(ndvr,nvib)
  real*8 t1,t2,fc_factor(nvib,nvib)
...
!q0=-g_coup1/(mass(1)*omg1**2)
!  call compute_eigfn(ndvr,nvib,q,mass(1),omg1,q0,phi_gr)
!q0=g_coup1/(mass(1)*omg1**2)
!  call compute_eigfn(ndvr,nvib,q,mass(1),omg1,q0,phi_ex)

!do i=1,ndvr
!write(300,'(20es15.7)')q(i)*1.d10,phi_gr(i,:)
!write(301,'(20es15.7)')q(i)*1.d10,phi_ex(i,:)
!enddo
!stop
...
do i=1,nvib
        do j=1,nvib
        q_vib_gr(i,j)=dot_product(phi_gr(:,i),(q(:)*phi_gr(:,j)))
        q_vib_ex(i,j)=dot_product(phi_ex(:,i),(q(:)*phi_ex(:,j)))
!        fc_factor(i,j)=dot_product(phi_gr(:,i),phi_ex(:,j))
        enddo
enddo


!do i=1,nvib
!write(6,*)9,i-1,fc_factor(10,i)
!enddo
!stop
...
V_k_old=V_k
  ...
  acc_qm=0.d0
  pot_en=0.d0
  do j=1,n_el           !contibution from electron only
    do i=1,nclass
...
!do i=1,2*nvib
!write(60,'(i,20es20.5)')i,Hamil_diab(i,1:5)/wave_to_J!,q_vib_ex(i-nvib,i-nvib)*1.d10
!enddo
!stop
...
if(state(1)>0)then
        H_diab_vib(:,:)=Hamil_diab(1:nvib,1:nvib)
        delH_dels_vib(:,:,1)=delH_dels(1:nvib,1:nvib,1)
else
        H_diab_vib(:,:)=Hamil_diab(nvib+1:2*nvib,nvib+1:2*nvib)
        delH_dels_vib(:,:,1)=delH_dels(1+nvib:nvib*2,1+nvib:nvib*2,1)
endif


si_adiab_neu_prev=si_adiab_neu
hamil_vib=H_diab_vib
call diag(hamil_vib,nvib,ens_vib,vect_vib,nvib)
  do i=1,nvib
    si_adiab_neu(:,i)=vect_vib(:,i)
    if(sum(si_adiab_neu(:,i)*si_adiab_neu_prev(:,i))<0.d0)si_adiab_neu(:,i)=-si_adiab_neu(:,i)
  enddo

V_k_vib=ens_vib

if(state(1)>0)then
acc_qm(1)=acc_qm(1)-sum(si_adiab_neu(:,state(vib_el))*matmul(delH_dels_vib(:,:,1),si_adiab_neu(:,state(vib_el))))/mass(1)
else
acc_qm(1)=acc_qm(1)-sum(si_adiab_neu(:,state(vib_el)-nvib)*matmul(delH_dels_vib(:,:,1),si_adiab_neu(:,state(vib_el)-nvib)))/mass(1)
endif
...
subroutine compute_dij_2state(x_hop,k,kp,dp)
  ...
  do i=1,nclass
    dp(i)=sum(si_adiab_neu(:,k)*matmul(delH_dels_vib(:,:,i),si_adiab_neu(:,kp)))
  enddo
  dp=dp/(V_k_vib(kp)-V_k_vib(k))
...
subroutine compute_vdotd
  ...
  do i=1,nvib
    do j=1,nvib
      W_overlap(i,j)=sum(si_adiab_neu_prev(:,i)*si_adiab_neu(:,j))
    enddo
  enddo
...
subroutine setup_parameters
  ...
  real*8 omg_max,delw,w,coup,pos,pos1
  ...
  do k=1,ndvr
    q(k)=-2d-10 + 4d-10*k/real(ndvr)
  enddo

!write(6,*)omg1_prime/(2*pi*clight)
!stop

!pos=-V_exothermicity/(2*g_coup1)
!pos1=g_coup1/(mass(1)*omg1**2)
!x(1)=pos1*g_coup2/(mass(1)*omg2**2)
!write(6,*)V_exothermicity/wave_to_J,(0.5d0*mass(1)*omg1**2*(pos-pos1)**2+0.5d0*mass(1)*omg2**2*(x(1)-pos*g_coup2/(mass(1)*omg2**2))**2)/wave_to_J
!write(6,*)0.5d0*g_coup1**2/(mass(1)*omg1**2)*(1.d0+g_coup2**2/(mass(1)**2*omg1**2*omg2**2))/wave_to_J
!write(6,*)1.d0/(1+dexp(-V_exothermicity/(kb*temperature)))
!do i=1,1000
!x(1)=-0.5d-10+1.d-10/1000.d0*i
!write(201,*)x(1)*1.d10,0.5d0*mass(1)*omg1**2*(x(1)-pos1)**2/wave_to_J,(0.5d0*mass(1)*omg1**2*(x(1)+pos1)**2+V_exothermicity)/wave_to_J
!enddo
...
!x(1)=g_coup2/(mass(1)*omg2**2)*g_coup1/(mass(1)*omg1**2)

do i=1,nvib
do j=1,nvib
        !if(i==j)H_diab(i,i)=(i-0.5d0)*hbar*omg1-g_coup2*q_vib(i,i)*x(1)+g_coup2**2*q2_vib(i,i)/(2*mass(1)*omg2**2)
        if(i==j)H_diab(i,i)=(i-0.5d0)*hbar*omg1+0.5d0*mass(1)*omg2**2*((x(1)-g_coup2*q_vib_gr(i,i)/(mass(1)*omg2**2))**2-x(1)**2)
        !if(i.ne.j)H_diab(i,j)=-g_coup2*q_vib(i,j)*x(1)+g_coup2**2*q2_vib(i,j)/(2*mass(1)*omg2**2)
        if(i.ne.j)H_diab(i,j)=0.5d0*mass(1)*omg2**2*((x(1)-g_coup2*q_vib_gr(i,j)/(mass(1)*omg2**2))**2-x(1)**2)

        !if(i==j)H_diab(i+nvib,i+nvib)=(i-0.5d0)*hbar*omg1-g_coup2*q_vib(i,i)*x(1)+g_coup2**2*q2_vib(i,i)/(2*mass(1)*omg2**2)+V_exothermicity
        if(i==j)H_diab(i+nvib,i+nvib)=(i-0.5d0)*hbar*omg1+0.5d0*mass(1)*omg2**2*((x(1)-g_coup2*q_vib_ex(i,i)/(mass(1)*omg2**2))**2-x(1)**2)+V_exothermicity
        !if(i.ne.j)H_diab(i+nvib,j+nvib)=0.5d0*mass(1)*omg2**2*(x(1)-g_coup2*q_vib(i,j)/(mass(1)*omg2**2))**2-0.5d0*mass(1)*omg2**2*x(1)**2
        if(i.ne.j)H_diab(i+nvib,j+nvib)=0.5d0*mass(1)*omg2**2*((x(1)-g_coup2*q_vib_ex(i,j)/(mass(1)*omg2**2))**2-x(1)**2)

        if(i==j) delV_dels(i,i,1)=-g_coup2*q_vib_gr(i,i)
        if(i==j) delV_dels(i+nvib,i+nvib,1)=-g_coup2*q_vib_ex(i,i)
        if(i.ne.j) delV_dels(i,j,1)=-g_coup2*q_vib_gr(i,j)
        if(i.ne.j) delV_dels(i+nvib,j+nvib,1)=-g_coup2*q_vib_ex(i,j)

enddo
enddo

!do i=1,nvib
!write(6,*)i-1,H_diab(i,i)/wave_to_J,H_diab(i+nvib,i+nvib)/wave_to_J
!enddo
!stop
...
subroutine frank_condon(i,j,fc)
  ...
  do k=1,n_dvr
    q(k)=-1.2d-10 + 2.4d-10*k/real(n_dvr)
    !x0=-(g_coup1-g_coup2*x(1))/(mass(1)*omg1_prime**2)
    x0=g_coup1/(mass(1)*omg1**2)
    phi1(k)=sho_eigfn(i-1,q(k),mass(1),omg1,x0)
    !x0=-(-g_coup1-g_coup2*x(1))/(mass(1)*omg1_prime**2)
    x0=-g_coup1/(mass(1)*omg1**2)
    phi2(k)=sho_eigfn(j-1,q(k),mass(1),omg1,x0)
  enddo
  delq=q(2)-q(1)
...
subroutine compute_eigfn(nbasis1,nquant1,q,mass,omg,x0,phi)
  implicit none
  integer,intent(in) :: nbasis1,nquant1
  real*8,intent(in)::q(nbasis1),mass,omg,x0
  real*8,intent(out)::phi(nbasis1,nquant1)
  integer i,j
